...
class TestBankQueue {
	...
	void test()throws Exception {
		...
		counters[0] = initializeQueue(1, 1, 3); //6
		counters[1] = initializeQueue(2, 1, 2); //7
		counters[2] = initializeQueue(1, 1, 3); //6
		counters[3] = initializeQueue(2, 1, 2); //7
	...
	void testDistribute01() throws Exception {
		DeQ[] counters = new DeQ[4];
		counters[0] = initializeQueue(1, 1, 6);   //1  2  3  4  5  6
		counters[1] = initializeQueue(2, 7, 13);  //7  8  9  10 11 12 13
		counters[2] = initializeQueue(1, 14, 19); //14 15 16 17 18 19
		counters[3] = initializeQueue(2, 20, 26); //20 21 22 23 24 25 26
		//6 12 19 25 13
		//
		//
		//
		...
		/*System.out.println(counters[0].size());
		System.out.println(counters[1].size());
		System.out.println(counters[2].size());
		System.out.println(counters[3].size());*/
	...
	void testDistribute03() throws Exception {
		...
		counters[0] = initializeQueue(2, 1, 2); //1  2
		counters[1] = initializeQueue(1, 3, 4); //3  4
		counters[2] = initializeQueue(2, 5, 6); //5  6
		counters[3] = initializeQueue(1, 7, 8); //7  8
		counters[4] = initializeQueue(2, 9, 10);//9  10
	...
	void testDistribute04() throws Exception {
		...
		counters[0] = initializeQueue(2, 1, 4);  //1  2  3  4
		counters[1] = initializeQueue(1, 5, 8);  //5  6  7  8
		counters[2] = initializeQueue(2, 9, 12); //9  10 11 12
		counters[3] = initializeQueue(1, 13, 16);//13 14 15 16
		counters[4] = initializeQueue(2, 17, 20);//17 18 19 20
		counters[5] = initializeQueue(1, 21, 23);//21 22 23
												 //
		DeQ special = new DeQLinkedList();
		BankQueue s = new BankQueue(counters, special);
		s.distribute();
